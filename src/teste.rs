...
use crate::par::Par;
use crate::respostaSN::RespostaSN;
use crate::sessao::Sessao;
use crate::sessao::TipoSessao;
...
impl Teste {
    ...
    pub fn carregar(nome: &str, nome_arquivo: &str) -> Teste {
        let arquivo = File::open(nome_arquivo).expect("Erro ao abrir arquivo de teste.");
        ...
        loop {
            ...
            if cenarios.len() > 1 {
                if let Err(erro) = cenarios[0].teste_variaveis(&cenarios[cenarios.len() - 1]) {
                    return Err(format!(
                        "{} Deseja começar a criar os cenários novamente?",
                        erro
                    ));
                }
            }
    ...
    pub fn escrever(&self) -> File {
        ...
        let mut arquivo = File::create(nome).expect("Erro ao criar arquivo.");
    ...
    pub fn ler_sessoes(&self) -> Vec<Sessao> {
        ...
        while !controle.eq(".") {
            ...
            let mut dados: Vec<Data> = Vec::new();
            controle = linhas.next().unwrap();
            while !controle.eq(".") && !controle.eq(".Sessao:") {
                dados.push(Data::get_dados(controle));
                controle = linhas.next().unwrap();
            }
    ...
    pub fn exportar(&self, sessoes: &Vec<Sessao>) {
        ...
        for sessao in sessoes.iter() {
            if let TipoDados::DadosProcessados(ref dados) = sessao.dados {
                let mut tempo_anterior: String = Sessao::data_string(dados[0].time);
                for dado in dados.iter() {
                    let tempo = Sessao::data_string(dado.time);
                    if tempo.eq(&tempo_anterior) {
                        Par::inserir(&mut pares, &dado.name, dado.value.as_string());
                    } else {
