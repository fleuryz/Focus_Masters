...
use crate::dados::Data;
use crate::dados::TipoDados;
use crate::teste::Teste;
use crate::teste::Tipos;
...
use self::scrap::{Capturer, Display};
...
impl Sessao {
    ...
    pub fn iniciar(&mut self, teste: &Teste, escolha_cenario: Option<usize>) {
        ...
        let executavel = format!("./Jogos/{}", &teste.executavel);
        ...
        let thread1 = thread::Builder::new().name("Jogo_".to_string()).spawn(move || {
            let mensagem_erro = format!("Erro ao executar o jogo: ({}).", executavel);
        ...
        let mut nome_video: String = format!("{}", self.arquivo_video);

        self.data_inicio = time::now();
        let thread2 = thread::spawn(move || {
            let mut filmagem = Command::new("./Python/salvarCam.py")
                .arg(nome_video)
                .spawn()
        ...
        let thread3 = thread::spawn(move || {
            let mut filmagem = Command::new("ffmpeg")
        ...
        for linha in String::from_utf8(info).unwrap().lines() {
            if let TipoDados::DadosBrutos(ref mut dados_brutos) = self.dados {
                println!("{}", linha);
                dados_brutos.push(Data::get_dados(linha));
            }
        }
    }
    ...
    pub fn iniciar_texto(&mut self, teste: &Teste) {
        ...
        let thread3 = thread::spawn(move || {
            ...
            let mut filmagem = Command::new("./Python/salvarTela.py")
                .arg(nome_video2)
                .spawn()
                .expect("Screen was not recorded");
    ...
    pub fn processar_video(&mut self) -> Option<()> {
        let mut novos_dados: Vec<Data> = Vec::new();
        match self.dados {
            TipoDados::DadosProcessados(_) => return None,
            TipoDados::DadosBrutos(ref mut dados) => {
                println!("{}", self.arquivo_video.as_str());
                println!("{}", Sessao::data_string(self.data_inicio));
                ...
                for linha in String::from_utf8(output.stdout).unwrap().lines() {
                    println!("{}", linha);
                    dados.push(Data::get_dados(linha));
                }
    ...
    pub fn exportar_video(&mut self) -> Option<()> {
        ...
        match self.dados {
            TipoDados::DadosProcessados(_) => {
                let programa_video = "./Python/exportarVideo.2.py";
                let output = Command::new(programa_video)
                    .arg(self.arquivo_video.as_str())
                    .arg(Sessao::data_string(self.data_inicio))
                    .arg("0")
            ...
            TipoDados::DadosBrutos(ref mut dados) => {
                println!(
                    "{} {:?}",
                    self.arquivo_video.as_str(),
                    Sessao::data_string(self.data_inicio)
                );
                let programa_video = "./Python/exportarVideo.2.py";
                let output = Command::new(programa_video)
                    .arg(self.arquivo_video.as_str())
